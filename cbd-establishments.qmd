...
***TODO: compare the following plots to what exists in cbd-employment-wages.qmd. Should I be focusing on establishments here instead?***

<b>

## Number of workers per employee size class in NC

Let's assume that the average number of workers in each employment size class is about the midpoint of the class. From the perspective of workers, what are the most common employer size classes? Let's guess that the midpoint for `n1000` is 2,000.

In this way we can look at the experience of a random employee in NC: what size employer might they work for? Perhaps unexpectedly, the employee is more likely to work for a company employing 1000+ than for an employer with 1-4 employees. About half of all employees in work or organizations that employ between 20 and 249.
...
## Aside: Noise added to CBD data to protect confidentiality

Since the US Census is statutorily required to avoid disclosing data that can be tied to an individual employer^[<https://www.census.gov/programs-surveys/cbp/technical-documentation/methodology.html> ], the CBP data includes noise, and the noise is larger (in percentage terms) where numbers are lower. CPB reports state totals along with totals by employee size class. If we sum the smaller employee categories, how far different are the results from the reported state totals?

```{r fig.height=8, fig.width=10}
#| label: fig-establishments-employee-count-quality-check
#| fig-height: 8
#| fig-width: 10
#| fig-cap: "Quality check: number of employees in NAICS category and amount of error"
...
data_for_plot |>
  ...
  guides(color = guide_legend(override.aes = c(linewidth = 3))) +
  labs(
    title = "As expected: cumulative distributions show\nsmaller numbers include more error",
    subtitle = glue("{comma(all_rows)} data points 2001-2021 (all US states)",
                    "; excluding any category with < 100 employees reported that year",
                    "\n{comma(n_no_cat_values)} ({percent(n_no_cat_values / all_rows)}) report no category values (only state-level value)",
                    "\n{comma(n_no_error)} ({percent(n_no_error / all_rows)}) report no error (category values sum to state-level value)",
                    "\n'pct diff' calculated as (stated - summed) / stated",
                    "\nmagnitude calculated as floor(log10(number_of_employees_in_state_in_naics_summary_category))"),

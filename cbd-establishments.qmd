...
```{r}
emp_size_class_levels = c("n1_4", "n5_9", "n10_19", "n20_49", "n50_99", "n100_249", "n250_499", "n500_999", 
                     "n1000_1", "n1000_2", "n1000_3", "n1000_4")

n_employees_ref <- tibble( # note: excluding n1000
  emp_size_class = emp_size_class_levels,
  emp_class_count = c(2, 7, 15, 35, 75, 175, 375, 750, 1250, 2000, 3750, 10000),
  factor_order = 1:12
)

d_county_totals <- d_cbp_data |>
...
d_sector_totals_state <- d_sector_totals_county |>
  summarize(est = sum(est, na.rm = TRUE),
            n1_4 = sum(n1_4, na.rm = TRUE),
            n5_9 = sum(n5_9, na.rm = TRUE),
            n10_19 = sum(n10_19, na.rm = TRUE),
...
d_state_totals <- d_sector_totals_state |>
  ...
  mutate(emp_size_class = as_factor(emp_size_class_levels),
...
d_usa_totals <- d_sector_totals_state |>
  summarize(est = sum(est, na.rm = TRUE),
            n1_4 = sum(n1_4, na.rm = TRUE),
            n5_9 = sum(n5_9, na.rm = TRUE),
            n10_19 = sum(n10_19, na.rm = TRUE),
...
The County Business Patterns data includes counts of establishments by number of workers. The category names embed the range. For example, `n1_4` includes establishments reporting 1-4 workers. Establishments with 1,000 or more employees in a county are encoded as follows:

```
N1000_1         Number of Establishments: Employment Size Class:
                      1,000-1,499 Employees

N1000_2         Number of Establishments: Employment Size Class:
                      1,500-2,499 Employees

N1000_3         Number of Establishments: Employment Size Class:
                      2,500-4,999 Employees

N1000_4         Number of Establishments: Employment Size Class:
...
data_for_plot <- d_sector_totals_state |>
  ...
  mutate(emp_in_size_class = value * emp_class_count,
         ...
         .by = "year") |>
  mutate(emp_size_class = fct_reorder(emp_size_class, factor_order), # reverse factors for plotting
         emp_size_class = as.ordered(emp_size_class),
...
data_for_plot <- d_sector_totals_county |>
  #filter(year == 2021) |>
  pivot_longer(n1_4:n1000_4,
               names_to = "emp_size_class",
               values_to = "value") |>
...
***TODO: check whether "1000 or more" applies at state level or county level; if the latter, may need to explicitly pull state-level data***

```{r fig.height=5, fig.width=7}
#| label: fig-state-establishments-pct-1000-plus
#| fig-height: 5
#| fig-width: 7
#| fig-cap: "Percent of establishments with 1000 or more employees 2001-2021"
#| column: page-inset-right
...
data_for_plot <- d_sector_totals_state |>
  ...
  #        .by = "year") |>
  filter(value > 0) |>
  filter(emp_size_class %in% c("n1000", "n500_999")) |> # 1000 or more employees
  
  mutate(emp_size_class = as.ordered(emp_size_class),
         naics_abbr = fct_lump(naics_abbr, n = 8, w = value))

data_for_plot |>
  filter(naics_abbr != "Other",
         ) |>
  ggplot() +
  geom_line(aes(x = year, y = value, color = naics_abbr, group = naics_abbr),
           linewidth = 0.4, alpha = 0.6) +
  facet_wrap( ~ emp_size_class, scales = "free_y") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
        ) +
  guides(color = guide_legend(override.aes = c(linewidth = 3))) +
  labs(
    title = "Percent of establishments with 1000\nor more employees 2001-2021",
    subtitle = glue(
      "Including next-lowest class 500-900 for comparison",
      "\nTop 8 sectors in largest 10 largest counties by employment (in 2021) as reported in CBP"),
    x = "",

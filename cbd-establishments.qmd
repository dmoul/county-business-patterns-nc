...
d_totals_state_nc <- d_cbp_data_state |>
  filter(fipstate == "37") |>
...
p1 <- d_totals_state_nc |>
  ...
  mutate(emp_size_class = factor(emp_size_class, emp_size_class_levels)) |> # -emp)) |> # reverse factors for plotting
  ggplot() +
...
data_for_plot <- d_sector_totals_state_nc |>
  ...
  mutate(emp_in_size_class = value * emp_class_count,
         ...
         .by = "year") |>
  mutate(emp_size_class = factor(emp_size_class, emp_size_class_levels),
         emp_size_class = as.ordered(emp_size_class),

...
```{r}
#| label: define-fips-and-naics-references
...
```{r}
#| label: define-inflation-employee-references
...
d_total_est_county <- d_cbp_data_county |>
...
d_total_est_state_nc <- d_cbp_data_state |>
  filter(fipstate == "37") |>
...
d_total_est_usa <- d_cbp_data_state |>
  #filter(fipstate == "37") |>
  inner_join(d_fips_state_ref,
             by = c("fipstate")) |>
  inner_join(d_naics_ref_2021,
...
d_emp_total_county <- d_cbp_data_county |>
  filter(fipstate == "37",
         naics == "------") |>
  select(year, fipscty, naics, emp) |> #, starts_with("n")) |>
  inner_join(d_fips_county_ref,
             by = c("fipscty")) |>
  inner_join(d_naics_ref_2021,
             by = "naics") |>
  inner_join(d_naics_abbr_ref,
             by = "naics_descr") |>
  # TODO: do I need to set emp_size_class factors?
  mutate(#emp_size_class = as_factor(emp_size_class), # keep existing order
         #emp_size_class = as.ordered(emp_size_class),
         pct_emp = emp / sum(emp),
         .by = c("year")) |>
...
# TODO: which of these are still needed / need to be changed?
...
d_sector_totals_county <- d_cbp_data_country |>
...
```{r}
#| label: define-top-naics-and-counties

top_naics_cutoff <- 5

top_naics <- d_emp_sector_total_state_nc |> # d_sector_totals_state |>
  filter(year == 2021) |>
...
data_for_plot <- d_emp_sector_total_state_nc |> #d_sector_totals_state_nc |>
  filter(year == 2021) |>
  #distinct(naics_abbr, .keep_all = TRUE) |>
  mutate(naics_abbr = fct_reorder(naics_abbr, emp)) 
...
data_for_plot <- bind_rows(
  d_emp_sector_total_state_nc |>
    filter(year == 2021) |>
    mutate(scope = "nc"),
    d_emp_sector_total_usa |> #d_sector_totals_usa
    filter(year == 2021) |>
    mutate(scope = "usa")
) |>
...
data_for_plot <- d_emp_state_nc_total_size_classes |> #sector_total_state_nc |>
  filter(year == 2021) |>
  inner_join(n_employees_ref |>
...
data_for_plot <- d_emp_state_nc_size_classes |> #d_sector_totals_state |>
  filter(year == 2021) |>
  inner_join(n_employees_ref |>
               select(-est_size_class),
             by = "emp_size_class") |>
...
p2 <- data_for_plot |>
  mutate(naics_abbr = fct_reorder(naics_abbr, emp, sum)) |> #-pct_emp, sum)) |>
  ggplot() +
...
```{r}
d_nc_2021_top5 <- d_emp_sector_total_county |> # d_sector_totals_county |>
  filter(year == 2021) |>
...
data_for_plot |>
  ...
  labs(
    title = glue("Employment trend: NC top {top_naics_cutoff} sectors by employment"),
    x = "",
...
data_for_plot |>
  ...
  expand_limits(y = 0) +
  labs(
    title = glue("Wage trend: NC average wages: top {top_naics_cutoff} sectors"),
...
## Wage trend: in final year are wages higer or lower than starting year?
...
data_for_plot |>
  ...
  labs(
    title = "NC average wage trend by sector 2001-2021",
    subtitle = glue("Is sector average wage in final year up or down compared to starting year in 2021 dollars?"),
    x = NULL,

...
In the state-level data for 2021 the CBP reported `r comma(sum(state_nc_workers_2021$emp))` workers in `r nrow(state_nc_workers_2021)` NAICS categories.
...
Further categorizing this data by industry sector reveals which industries have concentrations of workers in establishments of particular size classes. Employees in AccFood tend to work in establishments in the 10 to 100 employee range. In Retail there is a wider range: from very small to 499 employees and almost none larger. The industry sectors with the largest portion of employees working for the largest establishments are HeathSoc (hospital systems, I assume) and FinIns. Consruction and ProfSciTech have the largest portion of employees in the sectory working in 1-4 employee establishments.
...
## Trends in employment statewide
...
#| column: page-inset-right
#| fig-alt: "This line plot shows the percent of state employees working in the 10 largest counties by employment. The data ranges from 2001 to 2021."  

data_for_plot <- d_emp_total_county |> # emp in 2001 = 1
  ...
  mutate(pct_growth_since_2001 = emp / emp[year == min(year)],
         ...
         # xstart = lag(emp),
         # ystart = lag(pct_growth_since_2001),
         .by = c(fipscty))
...
n_smaller <- data_for_plot |>
  ...
  geom_line(aes(x = year, y = pct_growth_since_2001, group = county_name, color = pct_growth_full_period),
           linewidth = 0.4, alpha = 0.6, show.legend = FALSE) + #color = carolina_blue, 
  geom_text_repel(data = labels_for_plot,
  ...
  p2 <- data_for_plot |>
  filter(year == 2021) |>
  ggplot(aes(x = emp, y = pct_growth_full_period, color = pct_growth_full_period)) +
  ...
  scale_color_viridis_c(end = 0.85) +
  labs(
    subtitle = glue("The counties with the largest growth and decline are among the counties",
                    "\n with low-to mid-range number of workers"),
    x = "County employment in 2021 (log10 scale)",
    y = NULL
  )
  ...
  # p2 <- data_for_plot |>
  #   #filter(year == 2021) |>
  #   filter(county_name %in% c("Orange", "Durham")) |>
  #   ggplot() +
  #   geom_segment(aes(x = lag(emp, default = NA), xend = emp, y = lag(pct_growth_since_2001), yend = pct_growth_since_2001, 
  #                    color = year, group = county_name),
  #                size = 0.6, alpha = 0.6, na.rm = TRUE) +
  #   # geom_line(aes(x = emp, y = pct_growth_since_2021, color = year, group = county_name),
  #   #           size = 0.6, alpha = 0.6) + #, arrow = arrow(length = unit(2, "mm"), ends = "last")) + 
  #   scale_x_log10(labels = label_number(scale_cut = cut_short_scale())) +
  #   scale_color_viridis_c(end = 0.85) +
  #   labs(
  #     x = "County employment in 2021 (log10 scale)",
  ...
  p1 + p2 +
    plot_annotation(
      title = "County growth trajectories 2001-2021",
...
data_for_plot |>
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.position = "top") +
  labs(title = "NC employment by county 2001-2021", 
       subtitle = glue("Green cells indicate higher employment at end of period"),
       x = NULL,
       y = "Change in employment year-to-year",
       # fill = "Race",
       # color = "Race",
       caption = my_caption)
...
data_for_plot |>
  ggplot() +
  geom_rect(aes(xmin = min(year) - 1, xmax = max(year) + 1, 
                ymin = min(pct_growth_since_2001) - 0.01, ymax = max(pct_growth_since_2001) + 0.01,
                fill = fill_color),
            alpha = 0.02, show.legend = FALSE) +
  geom_hline(yintercept = 1, lty = 2, size = 0.2, alpha = 0.3) +
  ...
  theme(panel.grid = element_blank(),
        ...
        plot.title = element_text(size = rel(3.0), face = "bold"),
        legend.position = "top") +
  labs(title = "Year-to-year change in NC employment by county 2001-2021", 
...
d_emp_sector_total_state_nc |>
  filter(year == 2021) |>
  select(naics, naics_abbr, naics_descr, avg_wage_2021 = avg_wage, emp_2021 = emp, pct_emp_2021 = pct_emp) |>

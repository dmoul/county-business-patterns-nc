...
#   )
  ...
  # p2 <- data_for_plot |>
  #   #filter(year == 2021) |>
  #   #filter(county_name %in% head(county_name, 50)) |> #c("Orange", "Durham")
  #   ggplot() +
  #   geom_hline(yintercept = 1, lty = 2, size = 0.2, alpha = 0.2) +
  #   geom_path(aes(x = emp, y = pct_growth_since_2001, color = year, group = county_name),
  #             size = 0.6, alpha = 0.6) + #, arrow = arrow(length = unit(1, "mm"), type = "closed")) + #, ends = "last"
  #   geom_point(aes(x = emp, y = pct_growth_full_period, color = year),
